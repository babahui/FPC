...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 keep_rate=(1, ), fuse_token=False, jpm_layer=(False, )):
        super().__init__()
        ...
        # Initialize jpm_layer
        if len(jpm_layer) == 1:
            jpm_layer = jpm_layer * (depth - 1)
        jpm_layer = jpm_layer + (True, ) # last layer is jpm layer
        print('################', jpm_layer)
        self.jpm_layer = jpm_layer
        
        # Initialize keep_rate
        if len(keep_rate) == 1:
        ...
        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                keep_rate=keep_rate[i], fuse_token=fuse_token, jpm_layer=jpm_layer[i])
            for i in range(depth)])
    ...
    def forward_features(self, x, camera_id, view_id, base_keep_rate):
