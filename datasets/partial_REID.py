...
class Partial_REID(BaseImageDataset):
    ...
    def _process_dir(self, dir_path, relabel=False, data_type=''):
        ...
        if data_type == 'train':
            ...
            img_paths = glob.glob(osp.join(dir_path, '*.jpg')) # ./PartialREID/partial_body_images/001_001.jpg

            pid_container = set()
            for img_path in img_paths:
                pid = int(img_path.split('/')[-1].split('_')[0]) # pid就是图片名称
                pid_container.add(pid)
            pid2label = {pid: label for label, pid in enumerate(pid_container)}
